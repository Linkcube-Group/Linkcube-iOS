...
/*读取序列化非空数组(Library/Caches文件夹下)*/
+ (NSMutableArray *)loadCacheArray:(NSString *)path
{
    NSMutableArray *tempArray = [NSKeyedUnarchiver unarchiveObjectWithFile:path.cacheFilePath];
    if (!tempArray) {
        return [NSMutableArray array];
    }
	return tempArray;
...
/*读取序列化非空字典(Library/Caches文件夹下)*/
+ (NSMutableDictionary *)loadCacheDictionary:(NSString *)path
{
    NSMutableDictionary *tempDictionary = [NSKeyedUnarchiver unarchiveObjectWithFile:path.cacheFilePath];
    if (!tempDictionary) {
        return [NSMutableDictionary dictionary];
    }
	return tempDictionary;
...
{
    return path.createDocumentFileDirectory;
}

/*删除指定文件(Documents文件夹下)*/
+ (BOOL)deleteFile:(NSString *)path
{
    return path.deleteDocumentFile;
}

/*是否存在指定文件(Documents文件夹下)*/
+ (BOOL)fileExistsAtPath:(NSString *)path
{
    return path.isExistsDocumentFile;
}
